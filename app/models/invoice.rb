class Invoice < ApplicationRecord
  ...
  def merchant_invoice_discounted_revenue(merchant_id)
    (total_revenue_by_merchant(merchant_id) - total_discounts_by_merchant(merchant_id)).to_f/100
  end

  def merchant_invoice_revenue(merchant_id)
    total_revenue_by_merchant(merchant_id).to_f/100
  end

  def admin_invoice_total_revenue
    self.total_invoice_revenue.to_f/100
  end

  def admin_invoice_discounted_revenue
    (self.total_invoice_revenue - self.total_discounts).to_f/100
  end
end
